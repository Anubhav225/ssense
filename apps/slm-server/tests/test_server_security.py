...
try:
    ...
    raise ImportError(
        "security.py / memory_orchestrator.py failed to import (these have no vLLM/torch "
        f"dependency, so this should always succeed): {e}"
    )

# The HTTP integration tests need `main.app`, which pulls in engine.py -> vllm/torch.
# Those are heavy GPU-oriented deps that won't be installed in every environment
# (e.g. a laptop or CI runner without CUDA) - keep that import separate from the
# lightweight security/schema imports above so the heuristic/schema unit tests
# above still run (and matter) even when vLLM itself can't be imported here.
try:
...
class TestServerSecurityAndSchema(unittest.TestCase):
    ...
    def setUp(self):
        """Reset in-memory state between tests to prevent rate-limit/cache bleed.
        BUG FIX: this used to reference `memory_orchestrator.audit_cache` and
        `memory_orchestrator.rate_limiter._requests`, neither of which exist on
        MemoryOrchestrator (the real attributes are `_audit_hot` and
        `_chat_limiter`/`_reqs`) - every test in this file raised AttributeError
        before it ran any actual assertions.
        """
        asyncio.run(memory_orchestrator._audit_hot.delete("test_clear"))
        chat_limiter = memory_orchestrator._chat_limiter
        if hasattr(chat_limiter, "_reqs"):
            chat_limiter._reqs.clear()
