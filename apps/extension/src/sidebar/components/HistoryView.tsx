...
// Audits are kept for 3 months, then the server automatically discards them
// and re-runs a fresh audit next time you visit — matches AUDIT_TTL_SECONDS
// (90 days) in apps/slm-server/audit_store.py. Purely informational: the
// extension doesn't need to enforce this itself, just explain it so an
// older score isn't mistaken for a permanent rating.
...
export const HistoryView: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  return (
    <div className="ssense-root">
      ...
      <div style={{ margin: '10px 16px 0', padding: '8px 12px', borderRadius: 8, background: 'rgba(255,255,255,0.03)', border: '1px solid var(--ssense-border)', fontSize: 10.5, color: 'var(--ssense-text-muted)', lineHeight: 1.5 }}>
        Audits are kept for {AUDIT_TTL_DAYS} days, then automatically refreshed the next time you visit — a score never goes stale without you knowing.
      </div>
          ...
          return (
            ...
            >
              ...
              {(() => {
                ...
                return (
                  <div style={{ marginTop: 8, fontSize: 9.5, fontWeight: 600, color: fresh.stale ? 'var(--ssense-accent-amber)' : 'var(--ssense-text-muted)' }}>
                    {fresh.stale ? '↻ ' : '✓ '}{fresh.label}
                  </div>
                );
              })()}
            </div>
