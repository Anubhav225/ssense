...
In the modern digital economy, privacy has transitioned from a fundamental human right to a complex, multi-layered technical and legal challenge. Ssense is a production-grade privacy compliance platform designed to autonomously audit, enforce, and explain compliance with India's Digital Personal Data Protection (DPDP) Act 2023. Enforcement (fingerprint-spoofing, tracker removal, dark-pattern blocking) runs entirely in the browser with zero cloud dependency. Audit inference runs on a hardened FastAPI server; the extension ships pre-configured to reach it with no user setup, and all extraction of privacy policy text happens server-side — the raw text never passes through the browser, never reaches a database, and is discarded after a single inference call. An earlier version of this platform also shipped a bare-metal Rust native daemon for fully local, on-device inference using llama-cpp-rs and a 9B GGUF model; that component was removed to eliminate the 9 GB first-run download and signed installer requirement for a general public Chrome Web Store release. 

The system utilizes a specialized, dual-track Small Language Model (SLM), fine-tuned via advanced Distilled Preference Optimization (SimPO) to act as both a strict Forensic Legal Auditor and an empathetic Conversational Assistant. A highly privileged Chrome Manifest V3 (MV3) extension performs in-browser enforcement — actively intercepting invasive tracking mechanisms and blinding hardware fingerprinters before they execute — while audit and co-pilot inference run on a hardened FastAPI server utilizing in-process vLLM with PagedAttention, multi-LoRA adapter hot-swapping, a three-tier audit cache, and SHA-256 request coalescing. The server self-caps memory usage per hardware profile (discrete GPU, Jetson unified memory, or pure CPU), and the extension ships pre-configured to reach it with no user setup. This report provides an exhaustive architectural analysis of the system's design, focusing on the Machine Learning (ML) data forge, SLM training methodologies, Edge and Cloud execution environments, the browser extension enforcement mechanisms, overarching user workflow, and military-grade security infrastructure.
...
> the project.** `apps/native-daemon` no longer exists in this repository
> (nor does an offline/local-inference mode in the extension — it talks to
> the SLM server exclusively now). `libs/rust-utils` itself is still
> present on disk, but is now dead code with no consumer — it was only
> ever used by the daemon described below. See `docs/DEPLOYMENT.md` and
> `docs/SLM_Server_Architecture.md` for the current, live architecture.
> Kept below for historical context on the original design rationale only.
...
The server is deployed as a **single FastAPI container per hardware
profile** (`slm-server-gpu` / `-jetson` / `-cpu`, selected via Docker
Compose `profiles:`), fronted by an Nginx reverse proxy, with one shared
Redis instance used only for the distributed chat rate limiter — not a
4-tier design with separate `vllm-engine`/`qdrant-db` services. There is
no vector-database container: `rag_engine.py` memory-maps precomputed
`.safetensors` embeddings directly into the same process's RAM and runs
hybrid BM25 + dense (BGE) search in-process. See
`docs/SLM_Server_Architecture.md` for the full current breakdown.

### 6.2 Server Workflow & User Connection

Requests are cryptographically signed (HMAC-SHA256 over
`method:path:timestamp:nonce`) and sent to `main.py` via Nginx. There is
no job queue — FastAPI's async event loop handles the request directly,
and `/v1/chat/stream` opens an SSE stream (`text/event-stream`)
immediately; `/v1/audit(/by-url)` first checks a three-tier cache (hot
in-memory → SQLite → policy-hash match) and only reaches the vLLM engine
in-process on a genuine miss. As `engine.py` generates tokens for a chat
request, they stream back through the same SSE connection in real time.

### 6.3 High-Concurrency & Request Coalescing

To serve upwards of 10,000 concurrent *connections* within a self-imposed
32GB memory budget (see `engine.py::TARGET_TOTAL_MEMORY_GB`), the server
implements **SHA-256 request coalescing** for chat: the gateway hashes
`(response_mode, domain, prompt)`, and if an identical request is already
in flight, the new caller subscribes to the same in-flight generation's
broadcaster instead of triggering a second one — cutting redundant
compute when many users ask the same question at once. Audit requests
are deduplicated far more aggressively via the three-tier cache in §6.2,
since identical policy text should never re-run inference at all. A
simple in-process queue-depth counter (`SSENSE_MAX_QUEUE_DEPTH`, default
5000) acts as a circuit breaker, returning `503` once too many requests
are in flight — see `docs/SLM_Server_Architecture.md` §2.
