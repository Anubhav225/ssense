...
This data is then subjected to **SimPO (Simple Preference Optimization)** to align a highly efficient 7B parameter model (`Qwen2.5-7B-Instruct`). This approach yields an edge-deployable Small Language Model (SLM) capable of outperforming generic foundation models in strict Indian legal contexts, with a 0% jurisdictional contamination rate (zero GDPR/CCPA bias).
...
1.  **The Forensic Auditor (`train_audit.py`):**
    *   *Architecture:* Fine-tunes `Qwen2.5-7B-Instruct` at `Rank 128` (Alpha 256) to grant the neural network deep syntactic control over strict JSON schema generation.
...
*   **GGUF Q4_K_M Automation:** Immediately appended to the training cycle is an automated `export_gguf()` routine. This quantizes both adapters into 4-bit `q4_k_m` GGUF binaries, ensuring the adapters remain deployable on quantized/CPU-only runtimes. (Note: the current server loads bf16 LoRA adapters via vLLM directly; the GGUF export is retained for portability, not used by the deployed server.)
